...
import {
  LayoutDashboard, Calendar, Users, Mail, BarChart3, Settings,
  ChevronLeft, ChevronRight, LogOut, Menu, X, Sparkles, FileText, Heart
} from "lucide-react";
...
const navItems = [
  ...
  { label: "My Attendees", icon: Users, href: "/dashboard/attendees" },
  { label: "RSVPs", icon: Heart, href: "/dashboard/rsvps" },
  { label: "My Proposals", icon: FileText, href: "/dashboard/proposals" },
  { label: "Messaging", icon: Mail, href: "/dashboard/messaging" },
  ...
  return (
    <div className="min-h-screen flex bg-muted/40">
      ...
      <div className="fixed inset-0 -z-10 overflow-hidden pointer-events-none">
        ...
        <div className="absolute top-[-5%] right-[-5%] w-[500px] h-[500px] rounded-full opacity-[0.03]" style={{ background: "radial-gradient(circle, hsl(var(--primary)), transparent 70%)" }} />
        <div className="absolute bottom-[-5%] left-[-5%] w-[400px] h-[400px] rounded-full opacity-[0.03]" style={{ background: "radial-gradient(circle, hsl(var(--secondary)), transparent 70%)" }} />
      </div>
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-50 h-14 flex items-center px-4 justify-between bg-card/95 backdrop-blur-md border-b border-border/60 shadow-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="font-display font-bold text-primary text-sm">Aliko Organizer</span>
        </div>
        <div className="w-9" />
      </div>
